    ...
    <div class="content">
      ...
      >
        ...
        >
          <el-input
            size="small"
            v-model.trim=conditionForm.mac
          ></el-input>
        </el-form-item>
        <el-form-item
          label="IP"
      ...
      <div class="btn-wrapper">
        <el-button
          ...
          type="warning"
          @click="clearOffline"
        >{{$t('raidManage.clearOffline')}}</el-button>
      </div>
      <el-table
        :data="currentItems"
      ...
      >
        ...
        >
          <template slot-scope="props">
            ...
            <div
              v-else-if="props.row.server_code"
              @click="editRemark"
        ...
        >
          <template slot-scope="scope">
            <el-tag
              type="warning"
              v-if="scope.row.raid_state === 0"
            >{{$t('common.abnormal')}}</el-tag>
            <el-tag
              type="success"
              v-else-if="scope.row.raid_state === 1"
            >{{$t('common.normal')}}</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          prop="detail"
        ...
        >
          <template slot-scope="scope">
            <ul style="list-style: none;">
              <li
                class="device"
                v-for="detail in scope.row.detail.split(';').slice(0,-1)"
                :key="detail"
              >{{detail}}</li>
            </ul>
          </template>
...
export default {
  ...
  methods: {
    clearOffline () {
      this.$confirm(this.$ts('raidManage.clearOfflineConfirm'), { type: 'warning' }).then(() => {
      ...
      this.$http
        ...
        .then(res => {
          if (res.result === 1) {
            ...
            this.$message.error(this.$ts('raidManage.saveNoteFail'))
          }
        })
    },
    cancelEdit (row) {
    ...
    editRemark () {
      this.$message.info(this.$ts('raidManage.pleaseEditInServerManage'))
    },
    editNote (row) {
      row.editing = true
