  ...
  data() {
    return {
      ...
      itemFormRules: {
        ...
        url: [
          {required: true, message: this.$t('romManage.pleaseInputRomPath')}
        ],
        note: [
  ...
  methods: {
    confirmAllDownload() {
      if (!this.selectedItems.length) {
        this.$message.info(this.$t('romManage.selectOnePackage'))
        return false
      }
      if (this.selectedItems.length !== 1) {
        this.$message.info(this.$t('romManage.onlyOnePackage'))
        return false
      }
